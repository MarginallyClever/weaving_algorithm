...
final float stringAlpha = 64; // 0...255 with 0 being totally transparent.
...
void setup() {
  ...
  println("max lines="+totalLinesToDraw);
...
void inputSelected(File selection) {
  ...
  center = height*(float)upScale/2f;
...
void addDistanceWeight() {
  sobelPrecalculateDistances(
    sobelImage,
    0.2,  // low pass filter (hard edge of green circle)
